...
# filtrage des enregistrements avec chiens uniquement:
dogdata <- animdata[animdata$qui_pique == 'Chien',]
...
dogdata$date_piqure_saisie <- as.Date(dogdata$date_piqure_saisie)
...
# signalements
dogdata_idf <- dogdata[dogdata$departement_code %in% c("75","77","78",91:95),]
# semi meteo
DSKdata_idf <- DSKdata[DSKdata$departement_code %in% c("75","77","78",91:95),]

## 2.4.2 Création du subset pour l'AL:
# signalements
dogdata_al <- dogdata[ dogdata$departement_code %in% c("54","55","57","88","67","68"),]
# semi meteo
DSKdata_al <- DSKdata[ DSKdata$departement_code %in% c("54","55","57","88","67","68"),]

### 2.4.3 Création du subset pour RA:
# signalements
dogdata_ra <- dogdata[ dogdata$departement_code %in% c("01","07","26","38","42","69","73","74"),]
# semi meteo
...
# signalements
dogdata_winter17_long <- dogdata[dogdata$date_piqure_saisie >= "2017-10-01" & dogdata$date_piqure_saisie <= "2018-03-31",] 
dogdata_winter18_long <- dogdata[dogdata$date_piqure_saisie >= "2018-10-01" & dogdata$date_piqure_saisie <= "2019-03-31",] 
dogdata_winter19_long <- dogdata[dogdata$date_piqure_saisie >= "2019-10-01" & dogdata$date_piqure_saisie <= "2020-03-31",] 
dogdata_winter_long <-rbind(dogdata_winter17_long, dogdata_winter18_long, dogdata_winter19_long)
# semi meteo
...
# signalements
dogdata_winter17_short <- dogdata[dogdata$date_piqure_saisie >= "2017-11-01" & dogdata$date_piqure_saisie <= "2018-02-28",] 
dogdata_winter18_short <- dogdata[dogdata$date_piqure_saisie >= "2018-11-01" & dogdata$date_piqure_saisie <= "2019-02-28",] 
dogdata_winter19_short <- dogdata[dogdata$date_piqure_saisie >= "2019-11-01" & dogdata$date_piqure_saisie <= "2020-02-28",] 
dogdata_winter_short <-rbind(dogdata_winter17_short, dogdata_winter18_short, dogdata_winter19_short)
# semi meteo
...
rm(list = c(paste('dogdata_winter', 17:19, '_long', sep=''),
                paste('dogdata_winter', 17:19, '_short', sep=''),
...
ic_table_maker <- function(reportingdf, randomdf, paramvector, calcul){
  ...
  # avec filtrage par le vecteur vectornames sur le dataframe dogdata
...
weatherPlotGrid <- function(param, mode){
    ...
    datalist <- list('france'=list('name'='France', 'report'=dogdata, 'witness'=DSKdata_700avg ),
                     'idf'=list('name'='île-de-France', 'report'=dogdata_idf, 'witness'=DSKdata_700avg_idf ),
                     'al'=list('name'='Alsace', 'report'=dogdata_al,'witness'=DSKdata_700avg_al ),
                     'ra'=list('name'='Rhône-Alpes', 'report'=dogdata_ra, 'witness'=DSKdata_700avg_ra )
    )
    ...
    weatherPlot <- function(reportdata, witnessdata, region, param){
      ...
      # la donnée principale est tirée du dataframe dogdata qui représente la table de donnée 
      ...
      ggplot(reportdata, aes(x=date_piqure_saisie))+
        ...
        geom_vline(xintercept=as.Date(c( '2017-09-21', '2018-09-21', '2019-09-21')),
                   ...
                   size=.5)+
        #titre du graph
        ggtitle(paste('Profil temporel de "',paramname,'" associée aux ',nrow(dogdata),' lieux et dates de signalements comparés 
                               à ceux des mêmes dates mais pour un semis régulier de lieux: ',region,' de 2017-03-31 à 2020-04-01 -chiens'))+
        xlab(label = 'Date')+
        ...
        scale_y_continuous( breaks = seq(floor(min(dogdata[,param], na.rm = T)),
                                         ceiling(max(dogdata[,param], na.rm = T)),
                                         by=4),
                            limits = c(floor( min(dogdata[,param], na.rm = T)),
                                       ceiling(max(dogdata[,param], na.rm = T)) ) )+
...
# France entiere
france_quartile <- ic_table_maker(dogdata, DSKdata, vectornames, calcul='quartile')
datatable(france_quartile)
export_csv_table(france_quartile)

france_decile <- ic_table_maker(dogdata, DSKdata, vectornames, calcul='decile')
datatable(france_decile)
export_csv_table(france_decile)

france_centile <- ic_table_maker(dogdata, DSKdata, vectornames, calcul='centile')
...
h <- batch_histogram(dogdata, DSKdata_700avg, dsk_paramnames, dsk_paramnames)
weather_gridplot_h <- plot_grid(plotlist=h, labels = "AUTO", ncol=2, nrow = 3 , align = 'hv')
title_text_h <- paste('Profils météorologiques associés aux ',nrow(dogdata),' lieux et dates de signalements comparés à ceux des mêmes dates mais pour un semis régulier de lieux (France, Janvier 2017 – 5 Avril 2020), ',nrow(DSKdata_700avg),' jours -chiens')
